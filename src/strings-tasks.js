...
function startsWith(str, substr) {
  // return str.indexOf(substr) === 0 || false;
...
function countVowels(str) {
  ...
  for (let i = 0; i < vowels.length; i += 1) {
    for (let j = 0; j < strLowArr.length; j += 1) {
      if (strLowArr[j] === vowels[i]) res += 1;
    }
  }
  return res;
...
function isPalindrome(str) {
  const lowStr = str.replace(/\W/gi, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  let maxWord = '';
  strArr.forEach((item) => {
    if (item.length > maxWord.length) maxWord = item;
  });
  return maxWord;
}
...
function reverseWords(str) {
  ...
  const rev = strArr.map((item) => item.split('').reverse().join(''));
  const res = rev.join(' ');
  return res;
}
